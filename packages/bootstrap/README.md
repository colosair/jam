...
```bash
npx --yes @jam-mcp/bootstrap@1.1.0 init          # for a person
npx --yes @jam-mcp/bootstrap@1.1.0 setup --agent # for a coding agent, JSON only
```
...
needs neither, so `npx --yes @jam-mcp/bootstrap@1.1.0 <anything>` works when
nothing else does.
