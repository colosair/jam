...
```text
○ Checking @jam-mcp/launcher@1.4.2 on npm...
```
...
× Migration target is not available from the configured npm registry
  ...
  npm could not find @jam-mcp/launcher@1.4.2 in the configured registry.
...
› If you use it, run: codex mcp add jam -- npx --yes @jam-mcp/launcher@1.4.2 serve
```
...
npx --yes @jam-mcp/bootstrap@1.4.2 runtime --json
...
@jam-mcp/launcher@1.4.2` followed by that one command reaches the same place.
