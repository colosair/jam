...
```bash
npx --yes @jam-mcp/bootstrap@1.4.1 init          # for a person
npx --yes @jam-mcp/bootstrap@1.4.1 setup --agent # for a coding agent, JSON only
```
...
needs neither, so `npx --yes @jam-mcp/bootstrap@1.4.1 <anything>` works when
nothing else does.
