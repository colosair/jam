...
```bash
npx --yes @jam-mcp/bootstrap@1.7.0 init
```
...
```bash
npm install -g @jam-mcp/launcher@1.7.0
```

Without it, the same commands are `npx --yes @jam-mcp/launcher@1.7.0 <command>`.
...
```bash
npx --yes @jam-mcp/bootstrap@1.7.0 setup --agent --project PROJECT
```
...
Finish with `npx --yes @jam-mcp/bootstrap@1.7.0 status --json`.

## Commands

These read as `jam …` throughout — the short form from the optional global
install above. Everything works the same as `npx --yes @jam-mcp/launcher@1.7.0
…` if you skipped it.
...
{
  "mcpServers": {
    "jam": { "command": "npx", "args": ["--yes", "@jam-mcp/launcher@1.7.0", "serve"] }
  }
}
```
