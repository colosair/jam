...
```text
○ Checking @jam-mcp/launcher@1.4.6 on npm...
```
...
× Migration target is not available from the configured npm registry
  ...
  npm could not find @jam-mcp/launcher@1.4.6 in the configured registry.
...
› If you use it, run: codex mcp add jam -- npx --yes @jam-mcp/launcher@1.4.6 serve
```
...
```bash
npx --yes @jam-mcp/bootstrap@1.4.6 setup plan --json
npx --yes @jam-mcp/bootstrap@1.4.6 setup apply --non-interactive --json
npx --yes @jam-mcp/bootstrap@1.4.6 setup --agent
npx --yes @jam-mcp/bootstrap@1.4.6 doctor --json
npx --yes @jam-mcp/bootstrap@1.4.6 runtime --json
npx --yes @jam-mcp/bootstrap@1.4.6 auth status --json
```
...
@jam-mcp/launcher@1.4.6` followed by that one command reaches the same place.
