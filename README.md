...
```bash
npm install -g @jam-mcp/launcher@1.4.3
```

Without it, the same commands are `npx --yes @jam-mcp/launcher@1.4.3 <command>`.
...
install above. Everything works the same as `npx --yes @jam-mcp/launcher@1.4.3
…` if you skipped it.
...
{
  "mcpServers": {
    "jam": { "command": "npx", "args": ["--yes", "@jam-mcp/launcher@1.4.3", "serve"] }
  }
}
```
