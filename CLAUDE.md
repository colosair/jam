...
Jira issue key.** Before a key becomes a durable reference — a branch, a commit,
a PR, a document, a link — positively resolve that exact key against live Jira
with `jira_context`; an issue coming back is the resolution. A key JAM could not
read comes back in `meta.missingKeys`, and **it is not evidence that the number
is free**: a number nobody holds today can be minted tomorrow, and every
reference already written against it then points at somebody else's work.

`key` is a locator Jira can re-point; `issueId` is the immutable id and is on
every issue JAM returns. When a reference has to survive, record `issueId` beside
the key. To get a key for work that has no issue yet, create the issue
(`jira_write_plan` with `issue.create`, then `jira_write_apply`) and use the key
in the receipt. There is no other way to come by one.

The long form of this rule, with the failure it comes from, is in `AGENTS.md`
...
## Updating a machine that already has JAM

`jam update`, not `setup --agent`. Setup re-plans the project binding,
credentials and host registration together; an update moves the pin the host
launches JAM from and nothing else. It verifies the published build answers
before re-registering, reads the registration back, and finishes with `doctor` —
so a registration that lands but is not healthy is rolled back.

```bash
jam update              # or: npx --yes @jam-mcp/bootstrap@1.6.0 update
jam update check --json # what is registered, what is published; changes nothing
```

`UNKNOWN` means the registry could not be asked. It does not mean up to date.

JAM updates JAM only. ASC has `asc update`, and neither product calls the other;
if someone asks for both, run `jam update` first — ASC's work channel reads JAM —
then `asc update`, and report both.

To inspect before acting, the same three steps separately:

```bash
npx --yes @jam-mcp/bootstrap@1.6.0 setup plan --json                     # what would change; changes nothing
npx --yes @jam-mcp/bootstrap@1.6.0 setup apply --non-interactive --json  # execute it
npx --yes @jam-mcp/bootstrap@1.6.0 doctor --json                         # verify
```
...
```json
{
  "permissions": {
    "allow": [
      "Bash(npx --yes @jam-mcp/bootstrap@1.6.0 setup --agent)",
      "Bash(npx --yes @jam-mcp/bootstrap@1.6.0 runtime use package)"
    ]
  }
}
```
