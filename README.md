...
Without it, the same commands are `npx --yes @jam-mcp/launcher@1.4.4 <command>`.
...
install above. Everything works the same as `npx --yes @jam-mcp/launcher@1.4.4
…` if you skipped it.
...
{
  "mcpServers": {
    "jam": { "command": "npx", "args": ["--yes", "@jam-mcp/launcher@1.4.4", "serve"] }
  }
}
```
