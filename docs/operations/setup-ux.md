...
```text
○ Checking @jam-mcp/launcher@1.3.0 on npm...
```
...
× Migration target is not available from the configured npm registry
  ...
  npm could not find @jam-mcp/launcher@1.3.0 in the configured registry.
...
› If you use it, run: codex mcp add jam -- npx --yes @jam-mcp/launcher@1.3.0 serve
```
