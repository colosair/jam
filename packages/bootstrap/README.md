...
```bash
npx --yes @jam-mcp/bootstrap@1.3.2 init          # for a person
npx --yes @jam-mcp/bootstrap@1.3.2 setup --agent # for a coding agent, JSON only
```
...
needs neither, so `npx --yes @jam-mcp/bootstrap@1.3.2 <anything>` works when
nothing else does.
