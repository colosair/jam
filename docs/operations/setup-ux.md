...
```text
○ Checking @jam-mcp/launcher@1.4.4 on npm...
```
...
× Migration target is not available from the configured npm registry
  ...
  npm could not find @jam-mcp/launcher@1.4.4 in the configured registry.
...
› If you use it, run: codex mcp add jam -- npx --yes @jam-mcp/launcher@1.4.4 serve
```
...
```bash
npx --yes @jam-mcp/bootstrap@1.4.4 setup plan --json
npx --yes @jam-mcp/bootstrap@1.4.4 setup apply --non-interactive --json
npx --yes @jam-mcp/bootstrap@1.4.4 setup --agent
npx --yes @jam-mcp/bootstrap@1.4.4 doctor --json
npx --yes @jam-mcp/bootstrap@1.4.4 runtime --json
npx --yes @jam-mcp/bootstrap@1.4.4 auth status --json
```
...
@jam-mcp/launcher@1.4.4` followed by that one command reaches the same place.
