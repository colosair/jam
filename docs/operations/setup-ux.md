...
```text
○ Checking @jam-mcp/launcher@1.8.0 on npm...
```
...
× Migration target is not available from the configured npm registry
  ...
  npm could not find @jam-mcp/launcher@1.8.0 in the configured registry.
...
› If you use it, run: codex mcp add jam -- npx --yes @jam-mcp/launcher@1.8.0 serve
```
...
```bash
npx --yes @jam-mcp/bootstrap@1.8.0 setup plan --json
npx --yes @jam-mcp/bootstrap@1.8.0 setup apply --non-interactive --json
npx --yes @jam-mcp/bootstrap@1.8.0 setup --agent
npx --yes @jam-mcp/bootstrap@1.8.0 doctor --json
npx --yes @jam-mcp/bootstrap@1.8.0 runtime --json
npx --yes @jam-mcp/bootstrap@1.8.0 auth status --json
```
...
@jam-mcp/launcher@1.8.0` followed by that one command reaches the same place.
